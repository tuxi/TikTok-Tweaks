...
- (void)setupData {
    NSArray *items1 = @[
        [[XYConfigSettingsSwitchItem alloc] initWithTitle:@"无限制下载" isOn:[XYPreferenceManager shared].isUnlimitedDownload isOnChangeBlock:^(XYConfigSettingsSwitchItem *item, UISwitch *sw) {
            item.isOn = sw.isOn;
        } saveBlock:^(XYConfigSettingsSwitchItem *item) {
            [XYPreferenceManager shared].isUnlimitedDownload = item.isOn;
        }],
        [[XYConfigSettingsSwitchItem alloc] initWithTitle:@"自动播放下一个" isOn:[XYPreferenceManager shared].isAutoPlayNextVideoWhenPlayEnded isOnChangeBlock:^(XYConfigSettingsSwitchItem *item, UISwitch *sw) {
            item.isOn = sw.isOn;
        } saveBlock:^(XYConfigSettingsSwitchItem *item) {
            [XYPreferenceManager shared].isAutoPlayNextVideoWhenPlayEnded = item.isOn;
        }],
        [[XYConfigSettingsSwitchItem alloc] initWithTitle:@"开启首页纯视频模式" isOn:[XYPreferenceManager shared].isPureMode isOnChangeBlock:^(XYConfigSettingsSwitchItem *item, UISwitch *sw) {
            item.isOn = sw.isOn;
        } saveBlock:^(XYConfigSettingsSwitchItem *item) {
            [XYPreferenceManager shared].isPureMode = item.isOn;
        }],
        [[XYConfigSettingsSwitchItem alloc] initWithTitle:@"是否播放广告" isOn:[XYPreferenceManager shared].shouldPlayAds isOnChangeBlock:^(XYConfigSettingsSwitchItem *item, UISwitch *sw) {
            item.isOn = sw.isOn;
        } saveBlock:^(XYConfigSettingsSwitchItem *item) {
            [XYPreferenceManager shared].shouldPlayAds = item.isOn;
        }],
        [[XYConfigSettingsSwitchItem alloc] initWithTitle:@"一直显示进度条" isOn:[XYPreferenceManager shared].showProgressBar isOnChangeBlock:^(XYConfigSettingsSwitchItem *item, UISwitch *sw) {
            item.isOn = sw.isOn;
        } saveBlock:^(XYConfigSettingsSwitchItem *item) {
            [XYPreferenceManager shared].showProgressBar = item.isOn;
        }]
    ];
    
    XYConfigSettingsSection *sec1 = [[XYConfigSettingsSection alloc] initWithTitle:@"基础配置" items:items1];
    
    NSArray<XYCarrier *> *carriers = [XYPreferenceManager shared].carriers;
    
    NSMutableArray<XYConfigSettingsAreaItem *> *areaItems = [NSMutableArray arrayWithCapacity:carriers.count];
    NSString *currentCountryCode = [[[XYPreferenceManager shared] countryCode] lowercaseString];
    [carriers enumerateObjectsUsingBlock:^(XYCarrier * _Nonnull carrier, NSUInteger idx, BOOL * _Nonnull stop) {
        NSString *code = [carrier.code lowercaseString];
        NSString *title = carrier.area;
...
- (void)saveAction {
    ...
    if (selectedItem != nil && ![[XYPreferenceManager shared].countryCode.lowercaseString isEqualToString:[selectedItem.code lowercaseString]]) {
        NSString *t = [NSString stringWithFormat:@"区域已修改为%@", selectedItem.title];
        [XYPreferenceManager shared].countryCode = selectedItem.code;
        UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:t message:@"修改完成后，会重新app以完成配置" preferredStyle:UIAlertControllerStyleAlert];
